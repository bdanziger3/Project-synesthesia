...
import { fetchPlaylist, fetchUser, resetPlaylist, savePlaylistColor } from '../actions';
...
class selectPlaylistColor extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  // componentDidMount() {
  //   this.props.fetchUser('clarkinator_');
  // }
  componentDidMount() {
    ...
    next = () => {
      ...
      this.props.savePlaylistColor(this.state.sliderValue || 0);
      this.props.fetchPlaylist(this.props.user.spotifyID, this.state.sliderValue || 0, 20);
      this.props.history.push(`/playlistResults${this.props.history.location.search}`);
    };

    setStyle = () => {
      if (this.state.sliderValue === null) {
        return ({
          background: `linear-gradient( hsl(${this.state.sliderValue || 0}, 0%, 95%, 100%), hsl(${this.state.sliderValue || 0}, 0%, 70%, 100%))`,
        });
      } else {
        return ({
          background: `linear-gradient( hsl(${this.state.sliderValue || 0}, 100%, 95%, 100%), hsl(${this.state.sliderValue || 0}, 100%, 70%, 100%))`,
        });
      }
    };
    ...
    render() {
      return (
        <div id="background" style={this.setStyle()}>
          <div id="song-view">
            <div id="song-content">
              <h2>Select a color, and we will create a playlist of songs from your library we think you might associate with it.</h2>
              <ColorSlider
...
export default withRouter(connect(mapStateToProps, { fetchPlaylist, fetchUser, savePlaylistColor, resetPlaylist })(selectPlaylistColor));
